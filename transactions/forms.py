...
class SupplierForm(forms.ModelForm):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.fields['name'].widget.attrs.update({'class': 'textinput form-control', 'pattern': '[a-zA-Z\s]{1,50}',
...
class SaleForm(forms.ModelForm):
    codigo_seguimiento = forms.CharField(max_length=20, required=False, widget=forms.TextInput(attrs={'class': 'textinput form-control'}))
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.fields['name'].widget.attrs.update({'class': 'textinput form-control', 'pattern': '[a-zA-Z\s]{1,50}',
                                                 'title': 'Alphabets and Spaces only', 'required': 'true'})
        self.fields['phone'].widget.attrs.update({'class': 'textinput form-control', 'maxlength': '9',
                                                  'pattern': '[0-9]{9}', 'title': 'Numbers only', 'required': 'true'})
        self.fields['email'].widget.attrs.update({'class': 'textinput form-control'})
        self.fields['gstin'].widget.attrs.update({'class': 'textinput form-control', 'maxlength': '15',
                                                  'pattern': '[A-Z0-9]{15}', 'title': 'GSTIN Format Required'})
    ...
    class Meta:
        model = SaleBill
        fields = ['name', 'phone', 'address', 'email', 'gstin', 'codigo_seguimiento']
        widgets = {
            'address': forms.Textarea(
                attrs={
                    'class': 'textinput form-control',
...
class SaleDetailsForm(forms.ModelForm):
    class Meta:
        ...
        fields = ['destination', 'total', 'codigo_seguimiento']
